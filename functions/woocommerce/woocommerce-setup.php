...
if ( ! function_exists( 'progenitor_woocommerce_setup' ) ) :
  function progenitor_woocommerce_setup() {
    add_theme_support( 'woocommerce' );
  }
endif;
add_action( 'after_setup_theme', 'progenitor_woocommerce_setup' );
/*
...
// Add progenitor wrappers
add_action('woocommerce_before_main_content', 'progenitor_wrapper_start', 10);
add_action('woocommerce_after_main_content', 'progenitor_wrapper_end', 10);
function progenitor_wrapper_start() {
  echo '<main id="site-main" class="pt-5 pb-5">';
}
function progenitor_wrapper_end() {
  echo '</main>';
}
/*
Place a cart icon with number of items and total cost in the menu bar.
*/
function progenitor_woomenucart($menu, $args) {
	...
	ob_start();
		global $woocommerce;
		$viewing_cart = __('View your shopping cart', 'progenitor');
		$start_shopping = __('Start shopping', 'progenitor');
		$cart_url = $woocommerce->cart->get_cart_url();
		$shop_page_url = get_permalink( woocommerce_get_page_id( 'shop' ) );
		$cart_contents_count = $woocommerce->cart->cart_contents_count;
		$cart_contents = sprintf(_n('%d item', '%d items', $cart_contents_count, 'progenitor'), $cart_contents_count);
...
add_filter('wp_nav_menu_items','progenitor_woomenucart', 10, 2);
